...
import { Dropzone, MAX_MB, MAX_MINUTES, MIN_SECONDS } from "./Dropzone";
        ...
        <div className="border-b border-line px-6 py-5">
          <div className="mb-3.5 flex items-center gap-2.5">
            ...
            <span className="ml-auto text-xs text-ink-4">
              {mode === "file"
                ? `최대 ${MAX_MB}MB · ${MIN_SECONDS}초~${MAX_MINUTES}분`
                : `${MIN_SECONDS}초~${MAX_MINUTES}분`}
            </span>
          </div>
  ...
  return (
    <>
      ...
      <p className="mt-2.5 text-[12.5px] leading-[1.7] text-ink-4">
        {MIN_SECONDS}초~{MAX_MINUTES}분 · youtube.com / youtu.be 링크
      </p>
    </>
  );
}
