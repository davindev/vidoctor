...
#### Naver CLOVA Speech 검증 시도 (한국어 ASR 상용 SOTA, 결과: 부적합)

vlog 환각 천장을 뚫을 후보로 한국어 ASR 상용 SOTA인 **Naver CLOVA Speech (NEST 모델)**
도입을 검증. NCP 가입·Object Storage·도메인 생성·Secret Key 발급·sanity 호출까지 전 절차
진행.

**시도 절차**:
1. NCP 콘솔 → AI Services → CLOVA Speech 이용 신청
2. Object Storage 버킷 2개 생성 (`vidoctor-clova-input` / `vidoctor-clova-output`) —
   장문 인식 도메인이 Storage 경로 필수
3. 장문 인식 도메인 `vidoctor` 생성. 첫 호출 시 `speaker detect is off` 400 에러 →
   *long sentence 도메인은 화자 인식이 기본 활성*이어야 호출 받음 → 콘솔에서 화자 인식
   사용으로 변경
4. vlog 60초 클립(95~165s, filler 라벨 99~101s 포함)을 long sentence `/recognizer/upload`
   동기 호출
...
자동 제거**. `setting.fillerText: True` 옵션도 무시. 한국어 상용 SOTA ASR이 *깨끗한
transcript* 마케팅 정책 하에 학습돼 disfluency 보존이 *반대 방향* 우선순위.
...
이 검증의 가치는 *"v1.1 ASR 후보 후보군"의 잘못된 가지치기*: 상용 SOTA가 disfluency
검출엔 부적합함을 데이터로 확정해 v1.1 결정 시 *자체 fine-tune 또는 phoneme classifier*
방향으로 좁힘. NCP 가입 부담은 *부정적 결과*에도 회고 자료로 가치 변환.
...
- **상용 ASR SOTA가 도메인 task에 SOTA가 아닐 수 있다**. CLOVA Speech는 한국어 transcript
  품질이 SOTA지만 disfluency 검출 task엔 *반대 방향* 정책으로 부적합. "정확도 1위"가 *어떤
  metric에서* 1위인지 항상 확인 — 상용 솔루션 도입 의사결정 시 sanity check가 결정적.
