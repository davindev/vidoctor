...
2. *"강사 입장 좌하단→좌상단 이동, 시청자 입장 우하단→우상단"* → 합성 회전 입력으로 부호 컨벤션 박제 검증(`test_solve_head_pose_head_*`): yaw>0=강사 우측, pitch>0=강사 머리 아래. UI 가시 영역(`ui/app.py:_GAZE_DIRECTION_LABEL`)은 시청자 입장 한국어로 좌우 반전 표기.
