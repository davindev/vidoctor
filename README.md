...
- **기획서 §3 cps Precision ≥ 0.80 도달 실패.** 단순 σ·윈도우·trim lever로는 가까이도 못
  감. vlog FP 5개 중 z=2.7~3.0(cps 11~12) 케이스 2건은 라벨러가 놓친 "객관적으로 매우 빠른"
  구간으로 의심 — 즉 검출은 정당하지만 P 측정에 불리. 라벨 정밀도가 P 상한을 누른다.
- **too_slow는 σ-기반 분리가 본질적으로 어려움.** vlog 슬로우 라벨 z_min 분포(-0.81 ~
  -1.16)가 자연 발화 변동(짧은 머뭇거림·문장 종료 후 휴지)과 거의 겹침. 사용자 인지의
  "느림"은 cps 자체가 아니라 *맥락 기대 대비 느림*에 가깝고, 이는 영상 평균 z-score로
  표현되지 않음. → **slow detection은 신호 품질이 낮은 차원**임을 데이터로 확인.
- **lever 결합 시 부작용이 신호보다 클 수 있음.** Stage 1(SLOW=1.0)·Stage 6(trimmed
  std)은 단일로는 합리적 가정이지만 임계 체계 전체를 깨뜨려 FP 폭증. robust statistics
  도입은 임계 재보정과 짝을 이뤄야 하며, "한 번에 하나만"이 시간 면에서도 옳음.
- **filler 차원 의존이 cps에 도움.** filler.py가 신뢰성 있게 동작(F1 lecture 1.0 / vlog
  0.316)하니 cps에서 그 사전을 재사용해 noise 제거가 가능. 차원 간 의존이 항상 부담은
  아니며 검증된 시그널은 적극 활용 가치.
- **lecture는 cps 라벨 1개로 통계 의미가 작다.** P 변화 측정이 vlog 데이터에 의존 —
  lecture 라벨 보강 또는 lecture 카테고리에서 cps 활성화 자체 재검토가 다음 작업의 사전
  조건.
...
#### 시도 9 — 음향 lever 정밀 결합 재시도 (시도 3 실패 패턴 재검증)

**가설**: 시도 3의 v0(RMS만)·v1(RMS+F0)이 실패한 이유가 *임계 정밀화 부족*이라면,
*결합 조건*(ASR gap + voiced ratio + RMS + voiced run 길이)으로 차별화 가능.

**조건 sweep**:
- ASR 단어 사이 gap: 200ms ≤ gap ≤ 2초 (자연 호흡·긴 침묵 컷)
- gap 안 voiced ratio ≥ 40% (발화 존재 보장)
- gap 안 voiced run 길이 50~600ms (단음절 filler 길이 패턴)
- RMS ≥ 영상 75th percentile − 임계값

**결과**: 모든 임계 조합에서 **F1 0.05~0.17** — baseline 0.316보다 큰 폭 악화.
사전+음향 OR 결합도 F1 0.13~0.15.

**진단**: vlog 환경에 *ASR 토큰 사이 voiced gap*인 영역이 너무 다양 — 자연 호흡,
짧은 감탄·호명, 다른 사람 음성, 메인 화자 짧은 강조 발화. 결합 임계 정밀화로도
라벨러의 *머뭇거림 의도* 발화와 분리 불가능. 시도 3과 같은 패턴 재확인.

**채택 X**. 음향 신호 단독·결합은 vlog 환경의 본질적 분리 한계.
