...
- 음성을 단어 단위로 정확하게 받아쓴 뒤 머뭇거림 사전과 대조합니다. "어 어 어"처럼 같은 머뭇거림이 0.5초 안에 연속되면 하나로 묶어, 사용자에게 한 번의 머뭇거림으로 안내합니다.
...
업로드/유튜브 URL이 FastAPI SSE 엔드포인트로 들어오면 카테고리 분류와 R2 업로드를 병렬로 처리합니다. 이후 LangGraph가 5차원 분석을 실행하고 결과를 Supabase에 저장합니다. 각 분석 단계가 끝날 때마다 클라이언트에게 실시간으로 전달됩니다.
...
| **Semaphore 동시성 가드** | 무제한 동시 분석으로 인한 메모리 초과를 방지하는 데 사용합니다. WhisperX·MediaPipe 모델이 각 1~2GB의 RAM을 점유해 무제한 동시 실행 시 메모리 초과(OOM) 위험이 있어, 동시 분석을 2건으로 제한하고 한 건당 15분을 넘기면 강제 종료해 서비스 안정성을 확보합니다. |
...
| **uv + mise** | 의존성과 Python 버전을 고정하는 데 사용합니다. `mise install` 한 번으로 모든 개발 환경이 동일하게 맞춰집니다. |
...
| 차원 | F1 변화 | 핵심 변경 |
|---|---|---|
| **Filler** | • 강의 0.44 → **1.00**<br>• 브이로그 0.18 → 0.32 | KsponSpeech fine-tuned Whisper 교체 |
...
- **해결 2**: 머뭇거림 단어 사전을 다듬었습니다. 강의에서 논리 연결로 자주 쓰이는 "그러니까/그래서"는 머뭇거림이 아닌데도 잘못 잡히는 경우가 많아 제외했고, 청중 주의를 환기할 때 쓰는 "자"("자, 이제…")를 추가했습니다.
- **해결 3**: 사전 외 단어 반복 검출을 제거했습니다. 브이로그에서 "강아지! 강아지!" 같은 호명·강조 케이스가 많아, 인접한 같은 단어 반복을 발화 비유창성(disfluency)으로 보던 가정이 성립하지 않았습니다. **사전 단어의 인접 반복**만 하나의 구간으로 묶도록 정리했고, 사용자가 보는 검출 결과 노이즈가 11건 → 0건으로 줄었습니다.
- **해결 4**: 라벨은 1초 단위로 기록되어 있는 반면 모델 정밀도는 ±20ms로, 두 정밀도의 격차를 보정하기 위해 평가 허용 오차를 ±1초로 적용했습니다.
...
| 계층 | 한도 | 차단 방식 |
|---|---|---|
| **동시성 제한** | 동시 분석 2건 | `asyncio.Semaphore`로 동시 실행을 제한해 ML 모델 메모리 초과(OOM)를 방지합니다. |
