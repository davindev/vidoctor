...
2. *"강사 입장 좌하단→좌상단 이동, 시청자 입장 우하단→우상단"* → yaw 음수 = 강사 좌측 = 코드 "left" 매핑 일치 확인. 시청자 가시 표기 시 좌우 flip 검토는 v1.1.
...
- pitch는 라벨 변동 abs_max 5.9°가 일반 변동 abs_p95 6.1°와 분리 안 됨. 라벨이 pitch dominant인 케이스가 들어오면 임계 재조정 필요. 현재는 yaw 단독으로 잡힘.
- baseline 가정이 "강사 시간 대부분 정면 응시"에 의존. 영상 내내 시선 이탈하는 비정상 케이스는 가정 깨짐 (강의 도메인 가정상 무시).
- pitch 부호 컨벤션은 검증되지 않음. 사용자 ground truth "위로 올라감"인데 pitch 변화가 작아 진단 모호. 라벨 표본 확장 후 재검증.
