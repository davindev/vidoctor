...
5초 윈도우 / 1초 스텝. 200ms+ pause·filler 단어는 분모·분자에서 제외 — filler의 짧은
글자수 + 짧은 시간이 cps를 인공적으로 떨어뜨려 too_slow 오탐을 만들기 때문. 영상 평균
±σ 이탈로 판정. cps 단독으론 정상 발화의 빠름과 라벨러 인지 "속사포"가 분포에서 부분
겹쳐 분리 한계 — vlog는 F0(피치) AND 결합으로 P 끌어올림 (graph/nodes.py에서 분기).
"""
...
# 자연 단어간 휴지(50~200ms, 호흡·자음 클로저) vs 의미 있는 멈춤의 경계.
# 200ms 이상은 "진짜 멈춤"으로 분모에서 제외해 cps를 순수 발화 속도로 측정.
